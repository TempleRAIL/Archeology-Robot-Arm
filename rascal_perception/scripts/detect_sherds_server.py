...
class DetectSherdsServer:
    ...
    def __init__(self):
        ...
        sync.registerCallback( self.camera_data_callback )
        # Create server
        detect_sherds_server = rospy.Service('detect_sherds', SherdDetections, self.detect_sherds_callback)
    ...
    def camera_data_callback(self, im_msg, pc_msg):
        self.camera_data_lock.acquire()
        try:
            self.image_msg = im_msg
            self.point_cloud_msg = pc_msg
        finally:
    ...
    def segment_sherds(self, color_mask, sherds_img, non_sherds_img):
        """
    ...
    def locate_sherds(self, sherds_image, points, header):
        point_map = points 
    ...
    def detect_sherds_callback(self, req):
        # Check to make sure that data exists
        if self.image_msg is None or self.point_cloud_msg is None:
        ...
        self.camera_data_lock.acquire()
        try:
            img = self.bridge.imgmsg_to_cv2(self.image_msg, "bgr8")  # BGR OpenCV image
            # convert PointCloud2 ROS msg to numpy array (cloud corresponds to segmented sherds image)
            point_cloud = ros_numpy.numpify(self.point_cloud_msg)  # pointcloud should be dense (2D data structure corresponding to image)
            header = self.image_msg.header
        finally:
            self.camera_data_lock.release()
        rospy.logwarn("req.subtract_background value is: {}".format(req.subtract_background))
        if req.subtract_background:
            non_sherds_img = self.bridge.imgmsg_to_cv2(req.background_image, "bgr8")  # BGR OpenCV image
        else:
            non_sherds_img = None
        # Segment sherds using HSV color mask and non-sherds mask
        sherds_image = self.segment_sherds(req.color_mask, img, non_sherds_img)
        ...
        res.detections = self.locate_sherds(sherds_image, points, header)
        return res
