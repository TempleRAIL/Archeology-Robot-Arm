...
def image_callback(im_msg, pc_msg):
...
def unpack_mask(MultiArray_msg):
...
def callback_boundbox(sherds_image, points):
    ...
    detections.header = sherds_image.header	# meta-data
...
def detect_sherds_callback(req):
    ...
    res.detections = locate_sherds(sherds_image, points)
    return res
