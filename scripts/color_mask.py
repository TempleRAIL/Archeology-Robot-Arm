...
from std_msgs.msg import Bool, Int16MultiArray, MultiArrayLayout, MultiArrayDimension
...
##############################################################
# callback_make_mask(img_msg, Bool)
# This function detects and extracts a user-defined number of the most dominant colors in an image.
# inputs: sensor_msgs/Image and std_msgs/Bool
# publications: Float32MultiArray containg colors

def callback_make_mask(img_msg, Bool):
    calibration_trigger = Bool.data
    if not calibration_trigger:
    ...
    print ("Color mask published.")
    
    #color_img_sub.sub.unregister()
    #calibrate_trigger_sub.sub.unregister()
    print("Unsubscribed from color image and /Calibrate_Trigger.")
...
def color_mask():
    ...
    global color_img_sub, calibrate_trigger_sub
    
    color_img_sub = message_filters.Subscriber("/camera/color/image_raw", Image)
    print ("Subscribed to color image.")

    calibrate_trigger_sub = message_filters.Subscriber("/Calibrate_Trigger", Bool)
    print ("Subscribed to /Calibrate_Trigger.")

    sync = message_filters.ApproximateTimeSynchronizer([color_img_sub, calibrate_trigger_sub], 1, 0.1, allow_headerless = True)
    sync.registerCallback(callback_make_mask)

    #Image_msg = rospy.wait_for_message("/image_publisher_1587259753199592788/image_raw", Image)
